"""AST-based import boundary check for the Layer 2 routing subpackage.

Layer 2 (`skynetra.engines`) may import ONLY from itself and lower layers
(L0 `skynetra.foundation`, L1 `skynetra.domain`). The routing subpackage
...
under `skynetra/engines/routing/`.
...
ROUTING_ROOT = Path(__file__).resolve().parent.parent.parent / "skynetra" / "engines" / "routing"
...
def test_layer2_routing_files_exist() -> None:
    assert ROUTING_ROOT.is_dir()
    assert list(ROUTING_ROOT.rglob("*.py")), "skynetra/engines/routing must contain modules"


def test_layer2_routing_has_no_upward_imports() -> None:
    violations: list[str] = []
    for py_file in sorted(ROUTING_ROOT.rglob("*.py")):
        for module, lineno in _extract_imports(py_file):
            if module.split(".")[0] in FORBIDDEN_TOP_LEVEL_MODULES:
                violations.append(
                    f"{py_file.relative_to(ROUTING_ROOT.parent.parent.parent)}:{lineno} "
                    f"imports forbidden module '{module}'"
                )
    assert not violations, "\n".join(violations)


def test_layer2_routing_has_no_legacy_skynetra_layer_references() -> None:
    violations: list[str] = []
    for py_file in sorted(ROUTING_ROOT.rglob("*.py")):
        ...
        for line_no, line in enumerate(source.splitlines(), start=1):
            if any(token in line for token in FORBIDDEN_STRINGS):
                violations.append(
                    f"{py_file.relative_to(ROUTING_ROOT.parent.parent.parent)}:{line_no}: {line}"
                )
    assert not violations, "\n".join(violations)
